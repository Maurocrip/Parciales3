...
class Usuario implements IBM
{
    ...
    function __construct(string $correo, string $nombre, string $clave, int $id = -1, int $id_perfil = -1)
    {
        $this->_nombre = $nombre;
        $this->_correo = $correo;
        $this->_clave = $clave;
    ...
    static function TraerUnoJSON(string $clave, string $correo) 
    {
        ...
        while(!feof($ar))
        {
            ...
            {
                $objeto = json_decode($linea);
                if($objeto->clave == $clave && $objeto->correo == $correo)
                {
                    $objetoRetorno->usuario = new Usuario($objeto->correo,$objeto->nombre,$objeto->clave);
